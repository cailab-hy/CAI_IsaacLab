...
class AutomateEnvDisassembly(DirectRLEnvAutomate):
    ...
    def set_pos_inverse_kinematics(self, env_ids, close_gripper=False):
    ...
    def _disassemble_plug_from_socket(self):
        ...
        self._lift_gripper(self.disassembly_dists * 3.0)
        ...
        print(f"--- info for intersect ----------")
        print(f"curr pose: {self.fingertip_midpoint_pos}")
        print(f"held_z_pos: {held_z_pos}")
        print(f"fixed_z_pos+dist:{fixed_z_pos+self.disassembly_dists}")
        print(f"if_intersect: {if_intersect}")
        print(f"---------------------------------\n")
        
        # assemble_env_ids = np.argwhere(if_intersect==0).reshape(-1)
        # self._randomize_gripper_pose(assemble_env_ids, self.move_gripper_sim_steps, if_log=True, close_gripper=True)        

    def _lift_gripper(self, lift_distance):
    ...
    def _move_gripper_to_eef_pose(self, ctrl_tgt_pos, ctrl_tgt_quat, if_log, close_gripper):
        ...
        self.ctrl_target_fingertip_midpoint_pos[self.env_ids] = ctrl_tgt_pos[self.env_ids]
        self.ctrl_target_fingertip_midpoint_quat[self.env_ids] = ctrl_tgt_quat[self.env_ids]

        if if_log:
            self._log_robot_state_per_timestep()

        bad_envs = self.env_ids.clone()
        ik_attempt = 0

        while True:
            n_bad = bad_envs.shape[0]

            pos_error, aa_error = self.set_pos_inverse_kinematics(env_ids=bad_envs, close_gripper=close_gripper)
            pos_error = torch.linalg.norm(pos_error, dim=1) > 1e-3
            angle_error = torch.norm(aa_error, dim=1) > 1e-3
            any_error = torch.logical_or(pos_error, angle_error)
            bad_envs = bad_envs[any_error.nonzero(as_tuple=False).squeeze(-1)]
    ...
    def _save_log_traj(self):
        ...
        if len(self.log_arm_dof_pos) > 100: # self.cfg_task.env.num_log_traj:
            ...
            # Need to edit (Add by Seunghwan Yu)
            log_filename = os.path.join(os.getcwd(), self.data_dir, 'asset_00004_disassembly_traj.json')
