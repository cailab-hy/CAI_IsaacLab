...
class AutomateEnv(DirectRLEnv):
    ...
    def _apply_action(self):
        ...
        # Interpret actions as target pos displacements and set pos target (pos_threshold: [0.02, 0.02, 0.02])
        pos_actions = self.actions[:, 0:3] * self.pos_threshold     

        # Interpret actions as target rot (axis-angle) displacements (rot_threshold: [0.097, 0.097, 0.097])
        rot_actions = self.actions[:, 3:6]
        if self.cfg_task.unidirectional_rot: # False
            rot_actions[:, 2] = -(rot_actions[:, 2] + 1.0) * 0.5  # [-1, 0]
        rot_actions = rot_actions * self.rot_threshold

        # fingertip_midpoint_pos: current pos, pos_actions: delta pos
        self.ctrl_target_fingertip_midpoint_pos = self.fingertip_midpoint_pos + pos_actions
    ...
    def _set_assets_to_default_pose(self, env_ids):
        ...
        # fixed_state[:, 2] = 0.0  # Set the init z-value of fixed asset to 0.0 
        fixed_state[:, 7:] = 0.0
