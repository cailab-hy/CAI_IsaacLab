...
class DirectRLEnvAutomate(gym.Env):
    ...
    def step(self, action: torch.Tensor) -> VecEnvStepReturn:
        ...
        for _ in range(self.cfg.decimation):
            ...
            # === [1]. Logging the disassembly path for AutoMate (Edit by CAI-LAB) ==================
            # set actions into buffers (threshold: 20)
            if self.episode_length_buf[0] < 20:
                # [1-1]. Picking up the Plug for Disassembly Path
                self._apply_action()

            else:
                # [1-2]. Moving to a Random Position for Disassembly Path
                self._apply_randomize_action()

            # [1-3]. Logging the disaasembly path
            self._log_state_per()
            # =======================================================================================
        ...
        # === [2]. Saving the disassembly path for AutoMate (Edit by CAI-LAB) =================
    ...
    # === Function for Automate (Edit by CAI-Lab) ====================================================================
    @abstractmethod
